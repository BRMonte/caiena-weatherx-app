module OpenWeatherSdk
  class Services::FetchCurrentWeatherService < BaseService
    def self.call(city:)
      ...
      coordinates = OpenWeatherSdk::Services::FetchGeocodingService.call(city: city)
      if coordinates[:error]
        return build_error(ERROR_CODES[:geocoding_error], coordinates[:error][:message], retryable: false)
      end

      response = OpenWeatherSdk::Client.make_request(:current_weather, {
      ...
      OpenWeatherSdk::ResponseBuilder.new(response, 'Current Weather').parse_weather_response
