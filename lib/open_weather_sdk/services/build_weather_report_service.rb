module OpenWeatherSdk
  class Services::BuildWeatherReportService < BaseService
    ...
    def self.call(city:)
      ...
      current_weather = OpenWeatherSdk::Services::FetchCurrentWeatherService.call(city: city)
      ...
      forecast_data = OpenWeatherSdk::Services::FetchWeatherForecastService.call(city: city)
      if forecast_data[:error]
        return build_error(ERROR_CODES[:forecast_error], forecast_data[:error][:message], retryable: false)
      end
      
      daily_averages = OpenWeatherSdk::ForecastCalculator.calculate_daily_averages(forecast_data['list'])
